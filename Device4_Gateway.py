...
TRAFFIC = 'traffic'
PRIORITY_1_TRAFFIC = 100 * (10 ** 3)
PRIORITY_2_TRAFFIC = 100 * (10 ** 4)
PRIORITY_3_TRAFFIC = 100 * (10 ** 3)
PRIORITY_4_TRAFFIC = 100 * (10 ** 3)
    ...
    def addmission_control(self):
        ...
        if self.traffic[PRIORITY_1_PORT] - self.qos[PRIORITY_1_PORT][TRAFFIC] > self.base[PRIORITY_1_PORT]:
            ...
            if self.traffic[PRIORITY_2_PORT] - self.qos[PRIORITY_2_PORT][TRAFFIC] < self.base[PRIORITY_2_PORT]:
                ...
                # 2~4をドロップ
                if min([w4, w3, w2]) == w2:
                    switchhub.drop_flow(self.datapath, haddr_to_bin(PORT2_MAC))
                    self.dc[PRIORITY_2_PORT] += 1
                    #if self.qos[PRIORITY_2_PORT][QOS_FLAG] == QOS_ON:
                        #self.drop_flow(PRIORITY_2_PORT)
                        #self.del_qos(PRIORITY_2_PORT)
                elif min([w4, w3, w2]) == w3:
                    switchhub.drop_flow(self.datapath, haddr_to_bin(PORT3_MAC))
                    self.dc[PRIORITY_3_PORT] += 1
                    #if self.qos[PRIORITY_3_PORT][QOS_FLAG] == QOS_ON:
                        #self.drop_flow(PRIORITY_3_PORT)
        ...
        # 1の通信帯域はあるが
        elif self.traffic[PRIORITY_1_PORT] - self.qos[PRIORITY_1_PORT][TRAFFIC] < self.base[PRIORITY_1_PORT]:
